...
public class DialogueManager : MonoBehaviour
{
    ...
    private Dictionary<string, int> _bgNameMap = new()
{
    { "stange.jpg", 0 },
    { "dom.jpg", 1 },
    { "ad.jpg", 2 }
    ...
    void Start()
    {
        _currentBackground = FindObjectOfType<Background>();
    ...
    private void ChangeCharacter()
    {
        ...
        if (index != -1)
        {
            ...
            else if (_expressionIndexMapCharacter.TryGetValue(expressionName, out int expressionIndex))
            {
                ...
                _characters[index].ChangeEmotion(expressionIndex);
            }
            else
                Debug.LogWarning($"Выражение '{expressionName}' не найдено в словаре!");
        }
    }
    ...
    private void ChangeBackground()
    {
        ...
        if (_bgNameMap.TryGetValue(bgKey, out int index))
            _currentBackground.ChangeBG(index);
        else
            Debug.LogWarning($"Ключ фона '{bgKey}' не найден!");
    }
